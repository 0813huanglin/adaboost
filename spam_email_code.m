...
x = 1000;                    %number of data for training
T = 1000;                    %number of round will take for boosting
RData = table2cell(readtable('spam_email.txt'));%%read data
RData = RData(2:end,:);
[Row,Col] = size(RData);    %calculate size of read data
Label = ones(x,1);
for a =1 : Row
    if  strcmp('ham',RData{a,Col})  %C is positive sample G is negativ sample
        Label(a,1) = 1;
    else 
        Label(a,1) = -1;
    end
end
TrainSet=[str2double(RData(1:x,1:Col-1)),Label(1:x,:)];    %att1...att1+label  training data
TestSet=[str2double(RData(x+1:Row,1:Col-1)),Label(x+1:Row,:)];%testing data
%% start training
