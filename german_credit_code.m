...
x = 400;                    %number of data for training
T = 20000;                    %number of round will take for boosting
RData = table2dataset(readtable('german_credit1.txt'));%%read data
...
[Row,Col] = size(DataM);    %calculate size of read data
Label = ones(x,1);
for a =1 : Row
    if  strcmp('good',RData{a,col})  %C is positive sample G is negativ sample
        Label(a,1) = 1;
    else 
        Label(a,1) = -1;
    end
end
TrainSet=[DataM(1:x,1:Col),Label(1:x,1)];    %att1...att1+label  training data
TestSet=[(DataM(x+1:Row,1:Col)),Label(x+1:end,1)];%testing data
%% start training
...
%% decision stump
    ...
    for dim=1:Col          %for each feature dim
        ...
        for i = 1: x                            %for each sample
            for dir = 1:2                       %choose direction
                ...
                for j = 1:x
                    if  sign(Predic(j))==sign(TrainSet(j,Col+1))%mark it if this predic is correct
    ...
    for s = 1:x                                     %calculate weight for each sample
        temp(s,1) = -1 * Alpha * TrainSet(s,Col+1).'*bestSClas(s,1);
    ...
    for c = 1:x
        ...
        if sign(AgtCls(c,1))~=TrainSet(c,Col+1)      %%compare supervised label and predicted label
...
%but if i use this classifier opposite way it will be a very good classifier
    testerr = 0;
    for q = 1 : Row-x
        if sign(TestSet(q,Col+1)) == -sign(aggClasTest(q,1)) %neg sign flip my classifier
